...
def dateExt(text, fileuuid, meta, stkid):
    ...
    #data uploaded
    #exclude 1st line, if date not then inclued again
    #dl1: exclude this
    for line in text.splitlines():
        if not "generated" in line and not "report date" in line and not "date/time" in line:
            finalText=finalText+"\n"+line
    print(finalText)
    #* for optional
    #delimiter should be [-,/ and space]
    # to do
    #for the month jan 2021 01/02/2021
    ...
    #Allow "to"
    #fullWord
    pattern1=r'\b(0?[1-9]|[12][0-9]|3[01])[- \/.,]\b(0?[1-9]|1[0-2]|jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)[- \/.,](\d{4}|\d{2})' #remove w+ and match only motnths
    ...
    pattern3=r'\b(\d{4}|\d{2})[- \/.,]\b(0?[1-9]|1[0-2]|jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)[- \/.,](0[1-9]|[12][0-9]|3[01])'
    match3 = re.findall(pattern3,finalText)
    print("matching with 3rd pattern")
    if len(match3)==2:
    ...
    pattern4=r"(?<!\S)(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)(\d{4})"
